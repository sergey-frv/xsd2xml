...
namespace XsdToXmlGenerator.Lib
{
    public class XmlGenerator
    {
        public string Generate(string xsdPath)
        {
            ...
            {
                string[] xsdFiles = Directory.GetFiles(xsdDirectory, "*.xsd", SearchOption.AllDirectories);
        ...
        private string GenerateStringFromRegex(string pattern)
        {
            try
            {
        ...
        private string GenerateSimpleTypeValue(XmlSchemaElement element, XmlSchemaSimpleType simpleType)
        {
            if (simpleType.Content is XmlSchemaSimpleTypeRestriction restriction)
            {
                ...
                if (enums.Any())
                {
                    return enums.First().Value ?? string.Empty;
                }

                var patternFacet = restriction.Facets.OfType<XmlSchemaPatternFacet>().FirstOrDefault();
                if (patternFacet != null && patternFacet.Value != null)
                {
                ...
                if (lengthFacet != null && lengthFacet.Value != null)
                {
                ...
                if (minLengthFacet != null && minLengthFacet.Value != null)
                {
